...
// ============================================
// INTERFACES (used by JSX — no API yet)
// ============================================
...
export default function DirecteurPlaqueDashboard() {
  ...
  // Map API marques to BrandDisplayData
  const brands: BrandDisplayData[] = (marquesRaw || []).map(mapMarqueToBrand)

  // Helpers derived from brands
  const getBrandRanking = (): BrandDisplayData[] =>
    [...brands].sort((a, b) => b.stats.objectiveRate - a.stats.objectiveRate)

  const getTotalEmployees = (): number =>
    brands.reduce((sum, b) => sum + b.employeeCount, 0)

  const getTotalDealerships = (): number =>
    brands.reduce((sum, b) => sum + b.dealershipCount, 0)

  // Derive groupKPIs from brands aggregate data
  ...
  const avgSatisfaction = brands.length > 0
    ...
    : 0
  const totalMarketShare = brands.reduce((s, b) => s + b.stats.marketShare, 0)

  const groupKPIs = {
    revenue: {
      current: totalRevenue,
      target: totalTarget > 0 ? Math.round(totalRevenue / (totalTarget > 0 ? totalTarget / totalTarget : 1)) : 0,
      growth: brands.length > 0
        ? Math.round(brands.reduce((s, b) => s + b.quarterlyGrowth, 0) / brands.length)
        : 0,
    },
    ebitda: {
      current: totalMargin,
      margin: totalRevenue > 0 ? Math.round((totalMargin / totalRevenue) * 1000) / 10 : 0,
      target: Math.round(totalMargin * 1.03),
    },
    volume: {
      current: totalSales,
      target: totalTarget,
      objectiveRate: totalTarget > 0 ? Math.round((totalSales / totalTarget) * 1000) / 10 : 0,
    },
    marketShare: {
      current: Math.round(totalMarketShare * 10) / 10,
      evolution: 0,
    },
    satisfaction: {
      nps: avgSatisfaction,
  ...
  // Build alerts from notifications API (no fallback to static data)
  const displayAlerts: GroupAlert[] = (notifData || []).map((n: any) => ({
    id: n.id,
  ...
  // Build challenges from defis API (no fallback to static data)
  const displayChallenges: GroupChallenge[] = (defisData || []).map((d: any) => ({
    id: d.id,
  ...
  // GroupPerformanceChart (uses inline data from this component)
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
        <ExecutiveStatCard
          ...
          subtitle={`Objectif: ${(groupKPIs.volume.target > 0 ? (groupKPIs.revenue.current / 1000000).toFixed(0) : 0)}M€`}
          icon={DollarSign}
          color="blue"
          trend={groupKPIs.revenue.growth > 0 ? "up" : groupKPIs.revenue.growth < 0 ? "down" : "stable"}
        ...
        <ExecutiveStatCard
          ...
          icon={PieChart}
          color="amber"
          trend={groupKPIs.marketShare.evolution > 0 ? "up" : groupKPIs.marketShare.evolution < 0 ? "down" : "stable"}
